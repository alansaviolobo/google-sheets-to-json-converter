...
try {
    ...
    Object.keys(dataSets).forEach(key => {
        fetchAndCacheData(dataSets[key], GOOGLESHEETURL + key);
    ...
    const errors = [];
    Object.keys(dataSets).forEach(key => {
        const data = fs.readFileSync(CACHE_DIRECTORY + dataSets[key].replace(/\s/g, '-') + '.csv', 'utf8');
        if(data.includes('#REF!')) {
            errors.push(`#REF! found in ${dataSets[key]}`);
        }
        if(data.includes('Loading')) {
            errors.push(`Loading found in ${dataSets[key]}`);
        }
